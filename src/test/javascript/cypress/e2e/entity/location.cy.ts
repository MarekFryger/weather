...
describe('Location e2e test', () => {
  ...
  const locationSample = { name: 'Bacon programming bandwidth', lat: 61855, lon: 43336 };
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      ...
      cy.get(`[data-cy="country"]`).type('Sudan').should('have.value', 'Sudan');

      cy.get(`[data-cy="lat"]`).type('90940').should('have.value', '90940');

      cy.get(`[data-cy="lon"]`).type('88706').should('have.value', '88706');

      cy.get(`[data-cy="timeZone"]`).type('Fort').should('have.value', 'Fort');
